...
出力するJSONファイルの構造を以下に示します。アニメーション名にはbActionのnameが入ります。位置、回転、拡縮といったオブジェクトのアニメーションは一つにまとめ、ボーン名のかわりに"Object"という名前で出力します。ボーンのアニメーションはボーンごとにまとめます。成分名はlocationX、quatWなどを出力します。
...
```
{
  "アニメーション名": {
    ...
    "objectAnimation": {
        "成分名": {          locationX、locationY、locationZ、rotationX、rotationY、rotationZ
           ※上記と同じ
        }
    }
  }
