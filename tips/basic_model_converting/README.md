...
*ThreeJSColladaParser.parseStaticGeometries関数*

1. 結果オブジェクト(SceneData)の作成

2. すべての固定モデルのパース (parseStaticGeometries関数)
    1. ループ開始：(ロード結果のオブジェクト).dae.geometries
...
2. 　固定モデルの取得 => mesh
2. 　頂点データの配列（インターリーブ配列）の作成
3. 　ループ：mesh.vertices
4. 　　頂点データの配列に頂点の座標値を追加(x, y, z)
4. 　　頂点データの配列に頂点の法線値を追加(x, y, z)
4. 　　頂点データの配列に頂点のテクスチャ座標値をすべて追加(u, v)
5. 　ループ終了
2. 　面の頂点インデクスの配列の作成
3. 　ループ：mesh.faces
4. 　　面の頂点インデクスの配列に点インデクスをすべて追加
5. 　ループ終了
14. 　メッシュデータの作成
15. 　メッシュデータを結果の配列に追加
7. ループ終了
...
1. JSONの開始 \{ の出力
1. modelsの開始 models: \{ の出力
3. ループ：再構成結果の配列(List<ConvertedModel>)
4. 　モデルの開始："モデル名" \{ 出力
5. 　頂点の単位あたりのサイズの出力
6. 　頂点データ（インターリーブ配列）の出力
7. 　面の頂点インデクスデータの出力
8. 　モデルの閉じ \} の出力
5. ループ終了
1. modelsの終了 \} の出力
